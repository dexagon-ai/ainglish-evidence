#!/usr/bin/env python3
"""Offline digest, balance, leakage, and exact-pair novelty audit for wave C."""
...
def main() -> None:
    ...
    for campaign, meta in index["campaigns"].items():
        ...
        calibration_text = "\n".join("\n".join([row["english"], row["ainglish"], row["question"], *[str(option) for option in row["options"]], str(row["answer"])]) for row in calibration).lower()
        ...
        if campaign == "same-kind":
            assert all("SHA-256" in row["english"] and "checkpoint" in row["english"] for row in scientific)
        report.append({"campaign": campaign, "scientific": 48, "calibration": 8, "prior_exact_triple_overlap": 0})
    print(json.dumps({"status": "ok", "campaigns": report, "scientific_items": len(report) * 48, "model_calls": 0, "governance_writes": 0}, indent=2))
