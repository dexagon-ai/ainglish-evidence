#!/usr/bin/env python3
"""Run and file the fresh they-number replication exactly once."""
...
def main() -> None:
    ...
    if spec["replicates_hash"] not in targets:
        raise SystemExit("REFUSING: fresh authenticated suggestions no longer route this original to Dexagon")
    ...
    if not original or original.get("settlement_state") != "awaiting" or proposal.get("stage") != "measured" or proposal.get("superseded_by"):
        raise SystemExit("REFUSING: the live proposal or target original changed before mint")
    ...
    print("START fresh they-number replication after authenticated suggestions and proposal read", flush=True)
