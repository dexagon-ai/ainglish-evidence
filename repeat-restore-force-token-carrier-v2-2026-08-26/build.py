#!/usr/bin/env python3
"""Build the fresh successor-specific affirmative token population."""
...
ACTORS = ("Asha", "Bram", "Cleo", "Dario", "Esme", "Farah", "Galen", "Hana")
EVENTS = (
    ("raise", "raised", "raised", "signal flag"),
    ("lower", "lowered", "lowered", "loading bridge"),
    ("activate", "activated", "active", "warning beacon"),
    ("deactivate", "deactivated", "inactive", "transfer relay"),
    ("mount", "mounted", "mounted", "archive volume"),
    ("unmount", "unmounted", "unmounted", "scratch volume"),
    ("seal", "sealed", "sealed", "sample chamber"),
    ("unseal", "unsealed", "unsealed", "review envelope"),
)
QUALIFIERS = ("north", "east", "south", "west", "upper", "lower", "inner", "outer")
...
def main() -> None:
    rows = []
    for event_index, (lemma, past, state, noun) in enumerate(EVENTS):
        for item_index, (actor, qualifier) in enumerate(zip(ACTORS, QUALIFIERS, strict=True)):
            ...
            rows.append({
                "item_id": f"v2-{event_index + 1:02d}-{item_index + 1:02d}",
                "form": form,
    ...
    form_counts = {form: sum(row["form"] == form for row in rows) for form in ("repeat-event", "restore-state")}
    family_counts = {
        lemma: {form: sum(row["predicate_family"] == lemma and row["form"] == form for row in rows) for form in form_counts}
        for lemma, *_ in EVENTS
    }
    assert len(rows) == 64 and form_counts == {"repeat-event": 32, "restore-state": 32}
    assert all(counts == {"repeat-event": 4, "restore-state": 4} for counts in family_counts.values())
    packet = {
        "kind": "dexagon.ainglish.repeat-restore-force-token-carrier.v2",
        "proposal_slug": SLUG,
        "metric": "token_delta",
        "test_set": rows,
    ...
    (ROOT / "token-items.json").write_text(json.dumps(packet, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    index = {
        "kind": "dexagon.ainglish.repeat-restore-force-token-freeze.v2",
        "proposal_slug": SLUG,
    ...
    print(json.dumps({"pairs": 64, "form_counts": form_counts, "items_sha256": packet["items_sha256"], "content_sha256": index["content_sha256"]}, indent=2))
