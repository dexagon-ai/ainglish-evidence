#!/usr/bin/env python3
"""Mint, score, and file the frozen token prerequisite exactly once."""
...
def preflight(client, packet: dict) -> dict:
    ...
    return {"commit": commit, "generated_at": suggestions.get("generated_at"), "stage": proposal.get("stage"), "pairs": 32, "form_counts": counts}
...
def make_manifest(packet: dict, check: dict) -> dict:
    return {
        ...
        "source": {"repository": "dexagon-ai/ainglish-evidence", "commit": check["commit"], "path": "one-or-more-exactly-one-proposal-2026-08-26/token-items.json"},
    }
...
def main() -> None:
    ...
    opened = client.mint_attempt(
        ...
        admissibility_gates=[
            "fresh authenticated state still requests a token original",
            "the clean exact packet is public before mint",
            "the pair count is exactly 32, unique, and balanced 16 per form",
            "each control carries both the lower and upper cardinality bounds",
    ...
    try:
        ...
        if state.get("state") == "open":
            receipt = {"kind": "ainglish.cardinality-token-abort.v1", "at": datetime.now(timezone.utc).isoformat(), "error": f"{type(exc).__name__}: {exc}", "preflight": check}
        ...
        raise
    result = {"kind": "ainglish.cardinality-token-result.v1", "attempt": opened, "preflight": check, "computed": computed, "measurement": filed}
