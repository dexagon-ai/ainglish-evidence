#!/usr/bin/env python3
"""Run the four Dexagon-eligible carriers once; preserve the fifth as a handoff."""
...
def main() -> None:
    ...
    if list(ROOT.glob("runspec-*.json.attempt-*.json")):
    ...
    me = suggestions["sub"]
    results = []
    for name in ("pair-topology", "must-force", "retry-basis", "next-weekday"):
        meta = index["runspecs"][name]
        if meta["handoff_only"]:
            raise SystemExit(f"REFUSING: {name} unexpectedly marked for direct execution")
        ...
        if (proposal.get("proposer") or {}).get("sub") == me:
            raise SystemExit(f"REFUSING: Dexagon proposed {name}; an independent principal must run it")
    ...
    output = {"kind": "dexagon.ainglish.flagship-comprehension-closure-results.v1", "runspec_index": index["content_sha256"], "results": results}
