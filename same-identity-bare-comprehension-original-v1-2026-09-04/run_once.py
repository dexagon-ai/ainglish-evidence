#!/usr/bin/env python3
"""Run and file the frozen same-identity resolving original exactly once."""
...
def pairs(manifest: dict) -> set[tuple[str, str]]:
    items = manifest.get("items") or manifest.get("test_set") or []
    return {
...
def main() -> None:
    ...
    print("RESOURCE PREFLIGHT", json.dumps(resource_preflight(), sort_keys=True), flush=True)
    ...
    work = [
        ...
        if row.get("metric") == METRIC and row.get("state") == "strengthen_evidence"
    ]
    if len(work) != 1:
        raise SystemExit("REFUSING: fresh proposal no longer requests a resolving comprehension original")
    offered = [
    ...
    if not offered:
        raise SystemExit("REFUSING: personalized suggestions do not currently offer this work")
    ...
    if any(row.get("state") not in {"completed", "aborted", "superseded"} for row in proposal.get("attempts") or []):
    ...
    candidate_pairs = pairs({"items": items})
    prior_pairs = set()
    for row in proposal.get("measurements") or []:
        detail = client.measurement(row["manifest_hash"])
        prior_pairs.update(pairs((detail.get("measurement", detail).get("manifest") or {})))
    overlap = candidate_pairs & prior_pairs
    if overlap:
        raise SystemExit(f"REFUSING: {len(overlap)} complete marked/bare pairs overlap prior evidence")
    ...
    result = {
        "kind": "dexagon.ainglish.same-identity-bare-comprehension-result.v1",
        ...
        "stratum_results": measurement.get("stratum_results"), "server_preflight": preflight,
    }
