#!/usr/bin/env python3
"""Mint, score, and file one fresh-input token replication exactly once."""
...
def validate_frozen(target: dict) -> dict:
    ...
    expected = 12 // len(forms)
    if len(pairs) != 12 or counts != {form: expected for form in forms}:
...
def make_manifest(name: str, target: dict, check: dict) -> dict:
    return {
        ...
        "replicates_hash": target["replicates_hash"],
        "models": ENCODINGS,
        "test_set": target["pairs"],
        "items_sha256": check["items_sha256"],
        "test_set_note": (
            "Twelve fresh complete operational messages, balanced by form. Each English arm "
            "states the target original's comparator meaning in the same genre; no prior complete "
            "metric pair is reused."
        ),
        "estimand": {
        ...
        "source": {
            ...
            "path": f"settlement-token-replications-v1-2026-09-01/items.py#{name}",
        },
        "evidentiary_limit": (
...
def score(manifest: dict, forms: list[str]) -> tuple[dict, dict]:
    ...
    all_deltas = [cell["delta"] for rows in cells.values() for cell in rows]
    payload = {
        "metric": "token_delta",
        "formula_version": 1,
        "value": value,
...
def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in TARGETS:
        raise SystemExit("usage: run_once.py {except_l|supersedes|empty}")
    name = sys.argv[1]
    target = TARGETS[name]
    ...
    opened = client.mint_attempt(
        ...
        estimand=(
            ...
            "tiktoken 0.13.0 of mean token_delta on 12 fresh complete messages, preserving "
            "the target original's comparator genre and aggregation."
        ),
        admissibility_gates=[
            ...
            "all 12 complete pairs are unique and have zero exact overlap with prior public pairs on this proposal",
            "each comparator is complete meaning-matched careful English in the target original's genre",
    ...
    try:
        ...
        if state.get("state") == "open":
            receipt = {
                "kind": "dexagon.ainglish.settlement-token-replication-abort.v1",
                "at": datetime.now(timezone.utc).isoformat(),
            ...
            aborted = client.abort_attempt(
                opened["attempt_id"],
                "settlement token replication harness failed before measurement emission",
                receipt,
    ...
    fresh_original = client.measurement(target["replicates_hash"])
    result = {
        "kind": "dexagon.ainglish.settlement-token-replication-result.v1",
        "attempt": opened,
        "preflight": check,
        "computed": computed,
        "measurement": filed,
        "post_write_original": {
            key: fresh_original.get(key)
            for key in (
                "confirmed", "settlement_state", "replication_count", "disagreement_count"
            )
        },
    }
        ...
        json.dumps(
            {
                ...
                "post_write_original": result["post_write_original"],
            },
            indent=2,
        )
    )
