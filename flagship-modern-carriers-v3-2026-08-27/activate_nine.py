#!/usr/bin/env python3
"""Atomically activate all nine modern carriers after the reader gate clears."""
...
SOURCES = [
    (V2_ROOT, ("clusivity", "addressee", "uncertainty", "delegation", "collectivity")),
    (ROOT, ("attribution", "deadline", "disjunction", "polarity")),
]
...
def activate(root: Path, template: dict, panel: list[dict]) -> dict:
...
def main() -> None:
    ...
    for root, keys in SOURCES:
        for key in keys:
            active[key] = activate(root, json.loads((root / f"{key}.template.json").read_text(encoding="utf-8")), panel)
    assert len(active) == 9
    ...
    with tempfile.TemporaryDirectory(dir=args.output_dir.parent, prefix=f".{args.output_dir.name}-") as temp:
        ...
        index = {
            "kind": "dexagon.ainglish.flagship-activation-index.v3",
            "outputs": outputs,
            "atomic_batch": True,
            "reader_calls": 0,
            "governance_writes": 0,
