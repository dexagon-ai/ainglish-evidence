...
def main() -> None:
    target = ROOT / "reserve-holdout.json"
    if target.exists():
        raise SystemExit("REFUSING: reserve-holdout.json already exists")
    phase_a = json.loads((ROOT / "phase-a-result.json").read_text())
    if phase_a.get("roster_ready"):
        raise SystemExit("REFUSING: phase A already produced a roster; reserve spend is unnecessary")
    known = tags()
    panel = []
