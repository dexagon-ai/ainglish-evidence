...
class ProspectiveSelectionTests(unittest.TestCase):
    ...
    def fake_post(path: str, payload: dict) -> dict:
        ...
        return {"capabilities": ["completion", "tools", "thinking"],
                "template": ProspectiveSelectionTests.TEMPLATE,
                "details": {
                    "family": "seed_oss", "families": ["seed_oss"],
                    "parameter_size": "36.2B", "quantization_level": "Q4_K_M", "format": "gguf",
                }}
    ...
    def test_missing_zero_thinking_template_marker_refuses_before_plan(self) -> None:
        def broken_post(path: str, payload: dict) -> dict:
            value = self.fake_post(path, payload)
            value["template"] = "$thinking_budget = 0"
            return value
        with mock.patch.object(build_candidate_plan, "get", side_effect=self.fake_get), \
