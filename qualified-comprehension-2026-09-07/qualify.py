...
def controls(prefix,n=14):
 rows=[]
 colours=['ochre','teal','violet','indigo','copper','ivory','scarlet']
 for i in range(n):
  order=colours[i%7:]+colours[:i%7];gold=colours[(2*i+1)%7]
  letter='ABCDEFG'[order.index(gold)]
  question='Which colour does the record establish? '+ ' '.join(f'{k}={v}.' for k,v in zip('ABCDEFG',order))+' H=not determined.'
  rows.append({'id':f'{prefix}-{i+6100}','detectable':f'The current label for tile {i+6100} states {gold}.',
   ...
   'question':question,'options':list('ABCDEFGH'),'answer':letter})
 return rows
def build():
 ...
 save_new(ROOT/'QUALIFICATION-PLAN.json',{'kind':'ainglish.sept7-qualified-original-readers.v1','maximum_calls':56,
  'service':BASE,'physical_gpu':'GPU-a30417b6-2cc4-220a-d34d-f76dd9444456','models':MODELS,
  'screen':'14 independently authored neutral colour records per reader, both known and unknown arms,8 opaque answer labels,each positive position twice. SDK>=.5 gap and>=.95 recovered headroom; separately>=.95 semantic accuracy per arm,zero truncations/off-option/absence/transport faults.',
 ...
 print('56 neutral calls planned; zero model calls so far.')
def run():
 ...
 with Journal(ROOT/'qualification/calls.jsonl',{'freeze':commit,**plan}) as journal:
  ...
  def chat(reader,prompt):
   ...
   if truncated or raw.strip() not in list('ABCDEFGH'):faults.append(ordinal)
   return raw,truncated
  with patch.object(panel,'chat',side_effect=chat):
   for screen in screens:
    start_faults=len(faults)
    r=reader_qualification.run_screen(screen)
    semantic={cell:sum(o['answer']==(o['expected'] if cell=='detectable' else 'H') for o in r['observations'] if o['cell']==cell)/len(screen['controls']) for cell in ['detectable','other']}
