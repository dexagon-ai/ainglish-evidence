#!/usr/bin/env python3
"""Offline digest, balance, leakage, and novelty audit for learnability wave D."""
...
def prior_triples() -> set[tuple[str, str, str]]:
    ...
    for path in REPO.rglob("*.json"):
        if ROOT in path.parents:
            continue
        try:
            found.update(triples(json.loads(path.read_text())))
...
def main() -> None:
    ...
    for campaign, meta in index["campaigns"].items():
        payload = json.loads((ROOT / meta["items_path"]).read_text())
        rows = payload["items"]
        ...
        calibration_text = "\n".join(
            "\n".join([row["english"], row["ainglish"], row["question"], *map(str, row["options"]), str(row["answer"])])
            for row in calibration
        ).lower()
        assert not any(marker.lower() in calibration_text for marker in target_markers)
        entry = (ROOT / meta["entry"]["path"]).read_text()
