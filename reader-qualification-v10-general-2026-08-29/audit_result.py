#!/usr/bin/env python3
"""Recompute one general reader result and its append-only journal binding."""
...
def main() -> None:
    ...
    parser.add_argument("--seat", choices=("qwen", "seed"), required=True)
    parser.add_argument("--write", action="store_true")
    args = parser.parse_args()
    packet = checked(ROOT / "holdout.json")
    plan = checked(ROOT / f"holdout-{args.seat}-plan.json")
    result = checked(ROOT / f"holdout-{args.seat}-result.json")
    ...
    for row in semantic_rows:
        ...
        if not row["correct"]:
            errors.append({"item_id": row["item_id"], "axis": row["axis"], "expected_label": item["answer"], "observed_label": label})
    ...
    report = {
        ...
        "model_calls": 0,
        "network_calls": 0,
        "status": "passed-with-adverse-result" if not semantic_ok else "passed-with-qualified-result",
    }
    report["content_sha256"] = hashlib.sha256(canonical(report)).hexdigest()
    if args.write:
        target = ROOT / f"holdout-{args.seat}-result-audit.json"
        rendered = json.dumps(report, indent=2, ensure_ascii=False) + "\n"
        if target.exists() and target.read_text(encoding="utf-8") != rendered:
            raise SystemExit("REFUSING: frozen result audit drift")
