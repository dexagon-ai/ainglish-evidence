...
def build():
    controls=[]
    fields=['audible','glossy','occupied','valid_ticket','latched']
    terms=[['muted','audible'],['matte','glossy'],['empty','occupied'],['void','valid'],['unlatched','latched']]
    for i in range(16):
        ...
        controls.append({'id':f'exhibit-{1400+i}','gold':gold,'messages':[
            {'role':'system','content':'Interpret the exhibit record. Return one JSON object with exactly the five named boolean fields. A single JSON code fence is accepted, but no explanation or other content.'},
            {'role':'user','content':f'Exhibit record {1400+i}: '+facts+'. Required fields: '+', '.join(fields)+'. Key: audible means not muted; glossy means not matte; occupied means not empty; valid_ticket means valid rather than void; latched means not unlatched. Use true when that property is asserted, false for its stated opposite.'}]})
    plan={'kind':'ainglish.cached-reader-semantic-diagnostic.v1','governance_evidence':False,'models':MODELS,
        'options':{'num_gpu':0,'num_thread':8,'num_ctx':8192,'num_predict':512,'temperature':0,'seed':20260907},
        'answer_protocol':'Single exact boolean JSON object, bare or in one optional JSON fence, without additional commentary; duplicate or extra keys refused.',
        'predecessor':'cached-reader-diagnostic-2026-09-07 is retained unchanged. This successor addresses its wrapper failures with a new parser and freshly authored neutral controls before any target calls.',
        ...
        'analysis':'Declared bare-or-single-fence JSON protocol and exact boolean joint accuracy by language, dimension and authored context. Show absolute counts and format errors separately, no pooled cross-family success claim.',
        ...
        'limits':['Qwen GGUF differs from the earlier native NF4 reader; not a same-configuration repeat.',
            'Previously published synthetic cases, not a fresh independent holdout or human evidence. No reader from the predecessor reached these language targets.',
            'This is a prospectively changed interface, not retroactive conversion of the old qualification failures into passes.',
...
def run():
    ...
    for index,(model,digest) in enumerate(plan['models']):
        disk_guard()
        for _ in range(30):
            if ram()>=tags[model]['size']*1.1+4*1024**3:break
            time.sleep(1) # Wait only for pre-load resource release; never retry an inference call.
        else:raise RuntimeError('Insufficient available host memory before loading this cached model')
