#!/usr/bin/env python3
"""Mint, count, and file the frozen timezone token replication exactly once."""
...
TARGET = "287f3c7750198bd138a3582833b3c87d12c36f5f66c8bc1a7c9cb831de5e692b"
...
def preflight(client: Any, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    cards = suggestions.get("suggestions") or []
    card = next((row for row in cards if row.get("slug") == SLUG), None)
    if not card or card.get("replicates_hash") != TARGET or not card.get("executable_now"):
        raise RuntimeError("fresh personalised suggestions no longer route this exact target")
    ...
    if target.get("evidence_state") != "valid" or target.get("settlement_state") not in {
        "awaiting", "disputed"
    }:
        raise RuntimeError("target is no longer a valid unsettled original")
...
def main() -> None:
    ...
    opened = client.mint_attempt(
        ...
        + [
            "target remains a valid unsettled original",
