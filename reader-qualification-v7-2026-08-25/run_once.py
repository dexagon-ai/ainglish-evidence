#!/usr/bin/env python3
"""Run one frozen v7 tranche exactly once and retain every outcome."""
...
def validate(spec: dict, plan: dict) -> dict:
    ...
    for reader in spec["panel"]:
        ...
        source_capabilities = post("/api/show", {"model": reader["source_model"]}).get("capabilities")
        wrapper_capabilities = post("/api/show", {"model": reader["model"]}).get("capabilities")
        ...
        if "thinking" in source_capabilities or "thinking" in wrapper_capabilities:
            raise SystemExit(f"REFUSING: thinking-capable reader in no-thinking v7 panel: {reader['name']}")
