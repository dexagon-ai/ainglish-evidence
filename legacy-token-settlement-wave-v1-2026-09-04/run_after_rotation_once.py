#!/usr/bin/env python3
"""Execute frozen token carriers after they return to the live queue."""
...
OUTCOME = ROOT / "after-rotation-results.json"
...
def main() -> None:
    ...
    if OUTCOME.exists() or list(ROOT.glob("after-rotation-*.receipt.json")):
        raise SystemExit("REFUSING: this after-rotation run already has outcomes")
    ...
    for name, meta in index["campaigns"].items():
        ...
        outcome = {
            "kind": "dexagon.ainglish.legacy-token-after-rotation-receipt.v1",
        ...
        receipt = ROOT / f"after-rotation-{name}.receipt.json"
        receipt.write_text(json.dumps(outcome, indent=2) + "\n", encoding="utf-8")
        outcomes.append(outcome)
        print("FILED", name, json.dumps(outcome["result"], sort_keys=True), flush=True)

    output = {"kind": "dexagon.ainglish.legacy-token-after-rotation-results.v1", "outcomes": outcomes}
