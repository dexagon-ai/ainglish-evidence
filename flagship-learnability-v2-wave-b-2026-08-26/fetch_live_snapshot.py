#!/usr/bin/env python3
"""Freeze current semantic surfaces for the wave-B learnability carriers."""
...
PROPOSALS = {
    ...
    "fact-choice": "fact-not-known-choice-not-made-distinguish-missing-evidence-",
}
FIELDS = (
...
def main() -> None:
    ...
    for key, slug in PROPOSALS.items():
        ...
        if surface["slug"] != slug or surface["stage"] != "ratified" or surface["superseded_by"]:
            raise SystemExit(f"REFUSING: {slug} is not the current ratified surface")
        for field in ("title", "form", "english_mapping", "example_ainglish", "example_english"):
